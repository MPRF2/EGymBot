...
# === БАЗА УПРАЖНЕНИЙ ДЛЯ КОНСТРУКТОРA С КРАТКИМИ ЛАТИНСКИМИ КЛЮЧАМИ ===
CONSTRUCTOR_EXERCISES = {
    "ch_top": {
        "name": "Грудь (Верхний пучок)",
        "exercises": [
            "Жим штанги на наклонной скамье 30°", "Жим гантелей на наклонной скамье",
            "Жим в тренажере Смита на наклонной скамье", "Разведение гантелей на наклонной скамье",
            "Сведения рук в кроссовере снизу вверх", "Жим в наклонном хаммере на верх груди",
            "Жим гантелей хватом 'хаммер' под углом", "Наклонные отжимания от пола (ноги на скамье)",
            "Сведения рук на наклонной скамье в кроссовере", "Жим штанги обратным хватом на наклонной скамье",
            "Разведение рук с амортизатором снизу вверх", "Жим гири одной рукой на наклонной скамье",
            "Пуловер на наклонной скамье с гантелью", "Жим Свенда с блином перед собой под углом",
            "Сведения в наклонном тренажере Pec-Deck"
        ]
    },
    "ch_low": {
        "name": "Грудь (Средний/Нижний пучок)",
        "exercises": [
            "Жим штанги лежа горизонтально", "Жим гантелей лежа на горизонтальной скамье",
            "Жим в тренажере Hammer (Хаммер) на грудь", "Сведения в тренажере Пег-Дек (Баттерфляй)",
            "Отжимания на брусьях с акцентом на грудь", "Жим штанги на скамье с обратным наклоном",
            "Жим гантелей на скамье с обратным наклоном", "Сведения рук в кроссовере сверху вниз",
            "Отжимания от пола широким хватом", "Жим в Смите горизонтально",
            "Жим Свенда стоя с двумя блинами", "Сведения рук в кроссовере параллельно полу",
            "Отжимания от пола с весом на спине", "Жим в изоляционном тренажере на нижнюю часть груди",
            "Разведение гантелей на горизонтальной скамье"
        ]
    },
    "back_w": {
        "name": "Спина (Ширина / Лат.)",
        "exercises": [
            "Подтягивания широким хватом с весом", "Тяга верхнего блока к груди широким хватом",
            "Тяга верхнего блока параллельным хватом", "Пуловер на верхнем блоке с канатом / рукоятью",
            "Тяга блока за голову", "Тяга верхнего блока одной рукой сидя",
    ...
    "back_t": {
        "name": "Спина (Толщина / Плотность)",
        "exercises": [
            "Тяга штанги в наклоне (прямой/обратный хват)", "Тяга Т-грифа с упором в грудь",
            "Тяга гантели к поясу в наклоне одной рукой", "Горизонтальная тяга в Смите / рычажная тяга",
            "Становая тяга со стоек (Rack Pulls)", "Становая тяга классическая с пола",
            "Тяга Т-грифа без упора (свободный вес)", "Тяга Пендли (штанга с пола в каждом повторе)",
            "Тяга горизонтального блока к поясу узким хватом", "Рычажная тяга в Хаммере поочередно",
            "Гиперэкстензия с весом в руках", "Тяга двух гантелей в наклоне на скамье животом вниз",
            "Тяга Крока (тяжелая тяга гантели с читингом)", "Шраги со штангой перед собой (трапеции)",
            "Шраги с гантелями стоя"
        ]
    },
    "sh_mid": {
        "name": "Плечи (Средняя дельта — Пропорции)",
        "exercises": [
            ...
            "Махи гантелями в стороны сидя", "Махи одной рукой у нижнего блока",
            ...
            "Подъем блина в стороны двумя руками"
        ]
    },
    "sh_ant": {
        "name": "Плечи (Передняя/Задняя дельта)",
        "exercises": [
            "Жим гантелей сидя под углом 75-80°", "Армейский жим штанги стоя / сидя",
    ...
    "triceps": {
        ...
        "exercises": [
            ...
            "Кикбэк в кроссовере у нижнего блока", "Жим Смита узким хватом",
            "Отжимания от пола узким хватом (алмазные)"
        ]
    },
    "quads": {
        "name": "Ноги (Квадрицепс / Общий объем)",
        "exercises": [
...
async def process_const_muscle(c: types.CallbackQuery, state: FSMContext):
    ...
    b = InlineKeyboardBuilder()
    muscle_data = CONSTRUCTOR_EXERCISES.get(muscle_key, {"name": "Неизвестно", "exercises": []})
    
    for idx, ex in enumerate(muscle_data["exercises"]): 
        b.button(text=ex, callback_data=f"cex_{idx}")
        
    b.adjust(1)
    await c.message.edit_text(f"Группа: {muscle_data['name']}\nВыбери упражнение:", reply_markup=b.as_markup())
...
async def process_const_exercise(c: types.CallbackQuery, state: FSMContext):
    d = await state.get_data()
    muscle_key = d['chosen_muscle']
    ex_idx = int(c.data.split("_")[1])
    
    ex_name = CONSTRUCTOR_EXERCISES[muscle_key]["exercises"][ex_idx]
    muscle_real_name = CONSTRUCTOR_EXERCISES[muscle_key]["name"]
    
    await state.update_data(chosen_ex_name=ex_name, chosen_muscle_real_name=muscle_real_name)
